...
describe('Starting tests for angular-app', function() {
  ...
  it('navbar-brand should be decentralized-resource-network@0.1.1',() => {
    var navbarBrand = element(by.css('.navbar-brand')).getWebElement();
    expect(navbarBrand.getText()).toBe('decentralized-resource-network@0.1.1');
  });
    ...
    it('Resource component should be loadable',() => {
      page.navigateTo('/Resource');
      var assetName = browser.findElement(by.id('assetName'));
      expect(assetName.getText()).toBe('Resource');
    });

    it('Resource table should have 6 columns',() => {
      page.navigateTo('/Resource');
      element.all(by.css('.thead-cols th')).then(function(arr) {
